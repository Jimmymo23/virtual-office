    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
      <form onSubmit={handleCreate} style={{padding:'8px 12px',borderBottom:'0.5px solid #D3D1C7',display:'flex',gap:6}}>
        ...
        <button type="submit" disabled={submitting}
          style={{fontSize:10,padding:'5px 10px',borderRadius:6,border:'none',background:'#534AB7',color:'#fff',cursor:'pointer'}}>
          {submitting ? '...' : '+ create'}
        </button>
      </form>
      {error && <div style={{padding:'6px 12px',fontSize:11,color:'#A32D2D',background:'#FCEBEB'}}>{error}</div>}
      <div style={{flex:1,overflowY:'auto'}}>
        {offices.map(o => (
          <div key={o.id} style={{padding:'8px 12px',borderBottom:'0.5px solid #D3D1C7',display:'flex',alignItems:'center',gap:8}}>
            <div style={{flex:1}}>
              <div style={{fontSize:11,fontWeight:500,color:'#2C2C2A'}}>{o.name}</div>
              <div style={{fontSize:10,color:'#888780'}}>/{o.slug}</div>
            </div>
            <span style={{fontSize:9,padding:'1px 8px',borderRadius:20,background:'#F1EFE8',color:'#534AB7'}}>
